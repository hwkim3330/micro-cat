"""Part drawings (PDF) and BOM (CSV) from the parts report."""
...
c.setFont('Helvetica-Bold',26);c.drawString(40,H-60,'MICRO CAT Rev 1 — printed part reference drawings');c.setFont('Helvetica',12)
y=H-100
for line in ['Actuated chibi cat; 14 policy joints + jaw. Millimetres. Design pose (straight legs). Assembly frame: X forward, Y left, Z up, floor Z=0, trunk frame Z=120.',
...
c.showPage()
for p in parts:
    c.setFont('Helvetica-Bold',22);c.drawString(40,H-48,'MICRO CAT Rev 1 / '+p['name'])
...
with (R/'artifacts/bom.csv').open('w',newline='') as f:
    ...
    for p in parts:w.writerow([p['name'],1,'printed',p['body'],'PLA solid-equivalent volume; infill and material not frozen',p['mass_g'],'Rev 1 digital design',p['step']])
...
print('Wrote',len(parts),'drawing pages and the Rev 1 BOM')
