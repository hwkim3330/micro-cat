"""Geometry and evidence consistency checks for Micro Cat Rev B. Runs with numpy + trimesh only."""
...
assert (R/'models/micro_cat.glb').stat().st_size>1000 and (R/'models/micro_cat.step').stat().st_size>1000
...
xml=(R/'models/micro_cat_14.xml').read_text();assert 'trunk_base_freejoint' in xml and xml.count('<position ')==14 and 'mesh' not in xml
...
assert balance['model_sha256']==hashlib.sha256((R/'models/micro_cat_14.xml').read_bytes()).hexdigest(),'stale balance report'
