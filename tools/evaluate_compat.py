...
report=dict(scope='Unchanged official ONNX + published observation/action adapter + BAM M6, on an independent primitive X dynamics study. Not the P0 printable model, not a validated commercial robot.',interface_passed=True,observation_size=61,action_size=14,control_hz=50,model_sha256=hashlib.sha256((R/'models/micro_cat_14.xml').read_bytes()).hexdigest(),weights=json.loads((R/'engineering/policy_sources.json').read_text()),results=results,all_trials_upright=all(r['first_fall_s'] is None for r in results),physical_verified=False,complete_compatibility_verified=False)
