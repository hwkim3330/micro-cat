...
const root=path.resolve(import.meta.dirname,'..'),directory=await mkdtemp(path.join(os.tmpdir(),'micro-cat-lab-'));
...
try{
  ...
  for(const [name,width,height]of [['desktop',1440,1100],['mobile',390,844]]){
    ...
    let downloaded;for(let i=0;i<50;i++){try{downloaded=JSON.parse(await readFile(path.join(directory,'micro-cat-virtual-jaw-policy.json'),'utf8'));break;}catch{}await new Promise(r=>setTimeout(r,100));}assert.equal(downloaded.environment,'micro-cat-virtual-jaw-v1');checks++;
    ...
    assert.deepEqual(missing,[],'requests that 404ed: '+missing.join(', '));checks++;checks++;await page.close();await rm(path.join(directory,'micro-cat-virtual-jaw-policy.json'),{force:true});
  }
