"""Micro Cat local hardware bring-up service. MIT; no simulated camera fallback.
...
class Camera:
    ...
    def capture(self):
        ...
        try:
            with tempfile.TemporaryDirectory(prefix='micro-cat-capture-') as directory:
...
class Audio:
    ...
    def execute(self,record=False):
        ...
        try:
            with tempfile.TemporaryDirectory(prefix='micro-cat-audio-') as directory:
...
def health(camera):
    return {'service':'micro-cat-device','schema':1,'machine':platform.machine(),'camera':camera.probe(),
...
def main():
    ...
    server=make_server(args.port,camera,token,Audio(args.enable_audio,args.enable_microphone));print(f'Micro Cat local service: http://127.0.0.1:{server.server_port}; camera enabled={camera.enabled}',flush=True)
