...
class Environment:
    def __init__(self,standing=False,model_path=None,walking_path=None):
        self.model_path=Path(model_path or R/'models/micro_cat_14.xml')
        self.model_sha256=hashlib.sha256(self.model_path.read_bytes()).hexdigest()
        self.up=upstream()
        with contextlib.redirect_stdout(io.StringIO()):
            motor=self.up.load_bam_model(200,7.4,None)
            self.model,self.data,self.motor,_=self.up.load_mujoco_with_bam(str(model_path or R/'models/micro_cat_14.xml'),motor,.005,.1,6)
