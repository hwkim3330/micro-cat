"""Micro Cat Rev 1: actuated chibi cat, built in the straight-leg design pose.
...
def cone(p,axis,r0,r1,length):return cq.Workplane({'Z':'XY','Y':'XZ','X':'YZ'}[axis],origin=tuple(p)).circle(r0).workplane(offset=length).circle(r1).loft()
def zloft(sections):
    """Ellipse wires stacked along Z: (z, x centre, y centre, half X, half Y)."""
    return cq.Workplane(obj=cq.Solid.makeLoft([cq.Workplane('XY',origin=(x,y,z)).ellipse(a,b).val() for z,x,y,a,b in sections],ruled=False))
def curve_loft(nodes):
    """Ellipse wires on planes normal to the local centreline tangent: (x, z, half Y, half depth)."""
    P=np.array([[n[0],n[1]] for n in nodes],float);wires=[]
    for i,(x,z,a,b) in enumerate(nodes):
        t=P[min(i+1,len(P)-1)]-P[max(i-1,0)];t=t/np.linalg.norm(t)
        plane=cq.Plane(origin=(x,0,z),xDir=(0,1,0),normal=(float(t[0]),0.,float(t[1])))
        wires.append(cq.Workplane(plane).ellipse(a,b).val())
...
def build_leg(side):
    ...
    # Clip the bracket with the lower envelope of the chassis floor rotated to both roll extremes,
    # so the outboard rib cannot lift into the trunk skirt. Rev C clipped flat and jammed at +5.7 deg.
...
add('torso_shell',shell,SHELL,'trunk','Z','One-piece egg shell with a chest grille, neck slot and tail opening, open under the hips. Four M3 through the flanks into chassis bosses.',group='shell')
def loft(sections):
...
frame=frame.union(box(51,55,-8,8,244,DECK[0])).union(box(40.5,60,-8,8,244,248))         # post and forward beam, kept clear of the yaw channel at x 39.4
frame=frame.union(box(56,60,-14,14,230,254))                                           # camera wall
frame=frame.cut(pock('head_roll')).cut(pock('jaw'))
for y in [-10.5,10.5]:
    for z in [233.85,246.35]:frame=frame.cut(cyl((55,y,z),'X',1.15,6))
frame=frame.cut(cyl((55,0,241),'X',6.6,10))
for x in range(-10,42,14):frame=frame.cut(box(x,x+7,-11,11,DECK[0]-1,DECK[1]+1)) # lighten the deck
...
buy('camera_module_3',box(60,61.1,-12.5,12.5,230,254),DARK,'head',4,'Raspberry Pi Camera Module 3 Standard class, 25 x 24 mm board; the nose bore is a 14 mm funnel, so the usable field of view is about 60 deg, narrower than the module rating','camera')

# Skull: a round cranium loft plus a short muzzle ellipsoid, so the head reads as a cat and
# not as a beak or a snout. Cranium sections are (x, z centre, half width, half height).
SKULL=[(-30,252,20,17),(-14,253,42,30),(2,254,54,36),(22,254,55,37),(40,253,52,35),(54,251,44,29),(64,249,33,21),(72,247,18,11),(78,246,6,4)]
MUZZLE=(62,0,240);MUZZLE_R=(11,17,10) # a soft bump inside the round front, not a protruding snout
def skull_loft(shrink=0.0):
    return loft([(x,z,w-shrink,h-shrink) for x,z,w,h in SKULL]).union(ell(MUZZLE,tuple(v-shrink for v in MUZZLE_R)))
...
EAR=[(276,6,34,16,7),(288,8,35,13,5.5),(298,11,36,8.5,3.5),(306,13,36.5,3.5,1.6),(310,14,37,1.0,.7)]
for g in (1,-1):skull=skull.union(zloft([(z,x,g*y,a,b) for z,x,y,a,b in EAR]))
for x,y,z in BOSSES:
...
# ---------------------------------------------------------------- assembly, rig, report
print('assembly')
assembly=cq.Assembly(name='Micro_Cat_Rev1')
for p in parts+purchased:assembly.add(p['shape'],name=p['name'],color=cq.Color(*p['color']))
assembly.export(str(R/'models/micro_cat.step'))
scene=trimesh.Scene()
...
scene.export(R/'models/micro_cat.glb',include_normals=True)
report=dict(revision='Rev1 design-pose actuated chibi cat',parts=[{k:v for k,v in p.items() if k not in ['shape','mesh']} for p in parts],
