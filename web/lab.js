...
$('#replay-play').onclick=()=>{if(replayTimer){stopReplay();return}if(!replay||!robot())return;if(exploded)$('#explode').click();$('#replay-play').textContent='일시정지';replayIndex=0;
  replayTimer=setInterval(()=>{if(replayIndex>=replay.qpos.length)replayIndex=0;const q=replay.qpos[replayIndex];robot().setPose(q);view.controls.target.set(q[0],.15,-q[1]);robot().rig.qpos_layout.joints.forEach((name,i)=>{const row=$(`#joint-readout [data-joint="${name}"]`);if(row)row.textContent=`${JOINTS[name]||name}: ${(q[7+i]*180/Math.PI).toFixed(1)}°`});replayIndex++},replay.dt*1000)};
...
$('#export').onclick=()=>{if(!policy)return;const url=URL.createObjectURL(new Blob([JSON.stringify(policy,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download='micro-cat-virtual-jaw-policy.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)};
