// MIT. Shared loader that turns models/micro_cat.glb + models/rig.json into a poseable robot.
...
export async function loadRobot(base='../'){
  const [rig,parts,gltf]=await Promise.all([
    ...
    new GLTFLoader().loadAsync(base+'models/micro_cat.glb')]);
  const root=new THREE.Group();root.name='micro_cat';
